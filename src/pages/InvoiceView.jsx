...
export default function InvoiceView() {
  ...
  // Pre-filled with what is due, like the bill payment dialog.
  const openPay = () => { setPayForm({ date: today(), amount: amountDue > 0 ? String(amountDue) : '', bankAccountId: 'acc-cash', notes: '', exchangeRate: invRate }); setPayModal(true) }
        ...
        <div className="flex items-center gap-2">
          ...
          {etaOn && invoice.status !== 'void' && (
            <Btn variant="secondary" size="sm" onClick={exportEta} title={t('Download the ETA JSON document')}>
              ...
              {etaErrors.length > 0 && (
                <span className="ms-1 text-[10px] font-semibold px-1.5 py-px rounded-full bg-warning-100 text-warning-700 dark:bg-warning-500/20 dark:text-warning-300">
                  {etaErrors.length}
                </span>
              )}
            </Btn>
        ...
        <Card className="p-8 print:shadow-none">
          ...
          <DocumentHeader
            ...
            right={
              <div className="text-end">
                {zatca.enabled && (
                  <>
                    <p className="text-sm font-semibold text-slate-600 dark:text-slate-300" dir="rtl">فاتورة ضريبية مبسطة</p>
                    <p className="text-xs text-slate-500 dark:text-slate-400">{t('Simplified Tax Invoice')}</p>
                  </>
                )}
                <p className="text-2xl font-bold text-slate-800 dark:text-slate-100 mt-1">{invoice.number}</p>
                <Badge className={`mt-2 ${statusColor(invoice.status)}`}>
                  {invoice.status.toUpperCase()}
                </Badge>
                {zatca.enabled && zatca.crNumber && (
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">CR · السجل التجاري: {zatca.crNumber}</p>
                )}
              </div>
            }
          />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mb-8">
            <div>
              ...
              <p className="font-semibold text-slate-800 dark:text-slate-100">{invoice.customerName}</p>
              ...
              {customer?.taxId && <p className="text-sm text-slate-500 dark:text-slate-400">Tax ID: {customer.taxId}</p>}
              {customer?.address && <p className="text-sm text-slate-500 dark:text-slate-400 mt-1 whitespace-pre-line">{customer.address}</p>}
            </div>
            <div className="text-right space-y-2">
              <div>
                <p className="text-xs text-slate-500 dark:text-slate-400">{t('Invoice Date')}</p>
                <p className="font-medium text-slate-800 dark:text-slate-100">{fmtDate(invoice.date)}</p>
              </div>
              <div>
                <p className="text-xs text-slate-500 dark:text-slate-400">{t('Due Date')}</p>
                <p className="font-medium text-slate-800 dark:text-slate-100">{fmtDate(invoice.dueDate)}</p>
              </div>
              {invoice.salesRepId && (settings.salesReps || []).find((r) => r.id === invoice.salesRepId) && (
                <div>
                  <p className="text-xs text-slate-500 dark:text-slate-400">{t('Sales Rep')}</p>
                  <p className="font-medium text-slate-800 dark:text-slate-100">{(settings.salesReps || []).find((r) => r.id === invoice.salesRepId).name}</p>
                </div>
              )}
            </div>
          </div>
          ...
          <table className="w-full text-sm mb-6">
            <thead>
              <tr className="border-b-2 border-slate-200 dark:border-surface-700">
                <th className="text-left py-2 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">{t('Description')}</th>
                <th className="text-right py-2 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">Qty</th>
                <th className="text-right py-2 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">{t('Unit Price')}</th>
                {anyDiscount && <th className="text-right py-2 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">{t('Disc %')}</th>}
                {showTaxCol && <th className="text-right py-2 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">{t('VAT')}</th>}
                <th className="text-right py-2 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">{t('Amount')}</th>
              </tr>
            </thead>
            <tbody>
              {invoice.items.map((item) => (
                <tr key={item.id} className="border-b border-slate-100 dark:border-surface-750">
                  ...
                  <td className="py-3 text-right text-slate-600 dark:text-slate-300">{fmtMoney(item.unitPrice, invSym)}</td>
                  {anyDiscount && <td className="py-3 text-right text-slate-500 dark:text-slate-400">{(Number(item.discount) || 0) > 0 ? `${item.discount}%` : '—'}</td>}
                  {showTaxCol && <td className="py-3 text-right text-slate-500 dark:text-slate-400">{lineTaxLabel(item)}</td>}
                  <td className="py-3 text-right font-medium text-slate-800 dark:text-slate-100">{fmtMoney(item.subtotal, invSym)}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          {/* Amount in words */}
          <div className="mt-4 pt-3 border-t border-slate-100 dark:border-surface-750">
          ...
          {/* ZATCA QR code */}
          {zatcaOn && qrUrl && (
            <div className="mt-8 pt-6 border-t border-slate-100 dark:border-surface-750 flex items-center justify-between">
              <div className="text-sm">
                <p className="font-semibold text-slate-700 dark:text-slate-200">{t('ZATCA Compliant E-Invoice')}</p>
                <p className="text-slate-500 dark:text-slate-400" dir="rtl">فاتورة إلكترونية متوافقة مع هيئة الزكاة والضريبة والجمارك</p>
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">{t('Scan the QR code to verify this tax invoice.')}</p>
              </div>
          ...
          {invoice.payments?.length > 0 && (
            <div className="mt-6 pt-6 border-t border-slate-100 dark:border-surface-750">
              <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase mb-3">{t('Payment History')}</p>
              <div className="space-y-2">
                {invoice.payments.map((p) => (
                  <div key={p.id} className="flex justify-between text-sm">
                    <span className="text-slate-500 dark:text-slate-400">{fmtDate(p.date)} — {p.number}</span>
                    <span className="font-medium text-success-700 dark:text-success-400">{fmtMoney(p.amount, invSym)}</span>
                  </div>
                ))}
              </div>
            </div>
