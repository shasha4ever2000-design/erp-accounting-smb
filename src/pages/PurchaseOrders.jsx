...
export default function PurchaseOrders() {
  ...
  const handleDelete = async (po) => {
    if (await ask(`Delete PO ${po.number}?`)) deletePurchaseOrder(po.id)
  }
    ...
    <div>
      ...
      <Card>
        {purchaseOrders.length === 0 ? (
          ...
          <div className="py-12 text-center text-slate-500 dark:text-slate-400 text-sm">No POs with status "{filter}"</div>
        ) : (
          <Table headers={['PO Number', 'Supplier', 'Order Date', 'Delivery Date', 'Status', { label: 'Total', right: true }, { label: 'Actions', right: true }]}>
            {sorted.map((po) => (
              <Tr key={po.id}>
                <Td><span className="font-mono text-sm font-semibold text-warning-700 dark:text-warning-400">{po.number}</span></Td>
                <Td>
                  <p className="font-medium text-slate-900 dark:text-slate-100">{po.supplierName}</p>
                  {po.supplierEmail && <p className="text-xs text-slate-500 dark:text-slate-400">{po.supplierEmail}</p>}
                </Td>
                ...
                <Td right><span className="font-semibold text-slate-900 dark:text-slate-100 tabular-nums">{fmtMoney(po.total, sym)}</span></Td>
                <Td right>
                  <div className="flex justify-end gap-1">
                    ...
                    <Btn size="sm" variant="ghost" title={t('3-way match')} onClick={() => setMatchDoc(po)}>
                      <GitCompareArrows size={13} className="text-brand-600 dark:text-brand-400" />
                    </Btn>
                    {po.status !== 'cancelled' && po.status !== 'void' && canReceive(po) && (
                      <Btn size="sm" variant="ghost" onClick={() => setReceiveDoc(po)} title={t('Receive goods')}>
                        <PackageCheck size={13} className="text-success-700 dark:text-success-400" /> {t('Receive')}
                      </Btn>
                    )}
                    ...
                    {po.status === 'invoiced' && <span className="text-xs text-slate-500 dark:text-slate-400 px-2">{t('Billed')}</span>}
          ...
          return (
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-slate-500 dark:text-slate-400">{t('Purchase order → goods receipt → vendor bill')}</span>
              </div>
                ...
                <table className="w-full text-sm min-w-[480px]">
                  ...
                  <tbody>
                    {m.lines.map((l) => {
                      ...
                      return (
                        <tr key={l.id} className="border-b border-slate-50 dark:border-surface-700/50">
                          <td className="py-2 text-slate-700 dark:text-slate-200">{l.description || '—'}</td>
                          <td className="py-2 text-end px-2 tabular-nums text-slate-600 dark:text-slate-300">{l.ordered}</td>
                          <td className="py-2 text-end px-2 tabular-nums text-slate-600 dark:text-slate-300">{l.received}</td>
                          <td className="py-2 text-end px-2 tabular-nums text-slate-600 dark:text-slate-300">{l.billed}</td>
                          <td className="py-2 text-end ps-2 text-xs font-medium">
                            {flag ? <span className="text-danger-600 dark:text-danger-400">{l.overReceived ? t('Over-received') : t('Over-billed')}</span>
                              : ok ? <span className="text-success-700 dark:text-success-400">✓ {t('Matched')}</span>
                              : <span className="text-warning-700 dark:text-warning-400">{l.awaitingReceipt > 0 ? `${l.awaitingReceipt} ${t('to receive')}` : `${l.awaitingBill} ${t('to bill')}`}</span>}
                          </td>
                        </tr>
                      )
                    })}
