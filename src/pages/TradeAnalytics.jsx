...
function TopList({ title, icon, rows, sym, valueKey = 'value' }) {
  ...
  return (
    <Card className="p-5">
      <div className="flex items-center gap-2 mb-4">{icon}<h3 className="font-semibold text-slate-800 dark:text-slate-100">{title}</h3></div>
      {rows.length === 0 ? <p className="text-sm text-slate-500 dark:text-slate-400 py-4 text-center">—</p> : (
        <div className="space-y-2.5">
          {rows.map((r) => (
            <div key={r.name}>
              <div className="flex justify-between text-sm mb-1">
                <span className="text-slate-700 dark:text-slate-200 truncate pe-2">{r.name}</span>
                <span className="font-medium text-slate-900 dark:text-slate-100 tabular-nums flex-shrink-0">{fmtMoney(r[valueKey], sym)}</span>
              </div>
              <div className="h-1.5 rounded-full bg-slate-100 dark:bg-surface-700 overflow-hidden">
...
export default function TradeAnalytics() {
  ...
  return (
    <div>
      ...
      {/* Purchasing KPIs */}
      <h2 className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-3">{t('Purchasing')}</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      ...
      <Card className="p-5 mb-6">
        <h3 className="font-semibold text-slate-800 dark:text-slate-100 mb-4">{t('Sales vs Purchases — Last 12 Months')}</h3>
        <ResponsiveContainer width="100%" height={280}>
          <BarChart data={a.trend} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" vertical={false} className="stroke-slate-100 dark:stroke-surface-700" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-2">
        <TopList title={t('Top Customers')} icon={<Trophy size={16} className="text-brand-600 dark:text-brand-400" />} rows={a.sales.topCustomers} sym={sym} />
        <TopList title={t('Top Suppliers')} icon={<Trophy size={16} className="text-warning-700 dark:text-warning-400" />} rows={a.purch.topSuppliers} sym={sym} />
        <TopList title={t('Top Products by Revenue')} icon={<Trophy size={16} className="text-success-700 dark:text-success-400" />} rows={a.sales.topProducts} sym={sym} valueKey="revenue" />
      </div>
    </div>
  )
}
