...
export default function PurchaseView() {
  ...
  const t = useT()
  const {
    purchases, suppliers, accounts, journalEntries, settings, debitNotes, cashAccountOptions,
  ...
  return (
    <div>
      ...
      {editBlocked && editBlocked !== 'missing' && (
        <div className="max-w-3xl mx-auto mb-4 no-print text-xs rounded-lg px-3 py-2 bg-slate-50 dark:bg-surface-800/60 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-surface-700">
      ...
      <div className="max-w-3xl mx-auto">
        <Card className="p-8 print:shadow-none">
          <div className="flex items-start justify-between mb-8 gap-4">
            <div>
              <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">{t('Purchase Invoice')}</p>
              <p className="text-2xl font-bold text-slate-800 dark:text-slate-100 mt-1">{purchase.number}</p>
              {purchase.supplierRef && (
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">{t('Supplier ref')}: {purchase.supplierRef}</p>
              )}
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mb-8">
            <div>
              <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-2">{t('Supplier')}</p>
              <p className="font-semibold text-slate-800 dark:text-slate-100">{purchase.supplierName}</p>
              {supplier?.email && <p className="text-sm text-slate-500 dark:text-slate-400">{supplier.email}</p>}
              {supplier?.phone && <p className="text-sm text-slate-500 dark:text-slate-400">{supplier.phone}</p>}
              {supplier?.taxId && <p className="text-sm text-slate-500 dark:text-slate-400">{t('Tax ID')}: {supplier.taxId}</p>}
              {supplier?.address && <p className="text-sm text-slate-500 dark:text-slate-400 mt-1 whitespace-pre-line">{supplier.address}</p>}
            </div>
            <div className="text-end space-y-2">
              <div>
                <p className="text-xs text-slate-500 dark:text-slate-400">{t('Invoice Date')}</p>
                <p className="font-medium text-slate-800 dark:text-slate-100">{fmtDate(purchase.date)}</p>
              </div>
              <div>
                <p className="text-xs text-slate-500 dark:text-slate-400">{t('Due Date')}</p>
                <p className="font-medium text-slate-800 dark:text-slate-100">{fmtDate(purchase.dueDate)}</p>
              </div>
              {isFC && (
                <div>
                  <p className="text-xs text-slate-500 dark:text-slate-400">{t('Exchange rate')}</p>
                  <p className="font-medium text-slate-800 dark:text-slate-100">1 {purchase.currency} = {purRate} {baseCurrency}</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="overflow-x-auto -mx-1">
            <table className="w-full text-sm mb-6 min-w-[540px]">
              <thead>
                <tr className="border-b-2 border-slate-200 dark:border-surface-700">
                  <th className="text-start py-2 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">{t('Description')}</th>
                  <th className="text-start py-2 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">{t('Posts to')}</th>
                  <th className="text-end py-2 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">{t('Qty')}</th>
                  <th className="text-end py-2 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">{t('Unit Cost')}</th>
                  {anyDiscount && <th className="text-end py-2 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">{t('Disc %')}</th>}
                  {showTaxCol && <th className="text-end py-2 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">{t('VAT')}</th>}
                  <th className="text-end py-2 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase">{t('Amount')}</th>
                </tr>
              </thead>
              <tbody>
                {(purchase.items || []).map((item) => (
                  <tr key={item.id} className="border-b border-slate-100 dark:border-surface-750">
                    <td className="py-3 text-slate-700 dark:text-slate-200">{item.description}</td>
                    ...
                    <td className="py-3 text-xs text-slate-500 dark:text-slate-400">
                      {item.itemId
                        ? <span className="text-brand-600 dark:text-brand-400">📦 {t('Stock')}</span>
                        : accName(item.accountId)}
                    </td>
                    <td className="py-3 text-end text-slate-600 dark:text-slate-300">{item.quantity}</td>
                    <td className="py-3 text-end text-slate-600 dark:text-slate-300">{fmtMoney(item.unitPrice, purSym)}</td>
                    {anyDiscount && <td className="py-3 text-end text-slate-500 dark:text-slate-400">{(Number(item.discount) || 0) > 0 ? `${item.discount}%` : '—'}</td>}
                    {showTaxCol && <td className="py-3 text-end text-slate-500 dark:text-slate-400">{item.taxRate || 0}%</td>}
                    <td className="py-3 text-end font-medium text-slate-800 dark:text-slate-100">{fmtMoney(item.subtotal, purSym)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="flex justify-end">
            <div className="w-60 space-y-2 text-sm">
              {lineDiscount > 0.005 && (
                <>
                  <div className="flex justify-between text-slate-600 dark:text-slate-300"><span>{t('Subtotal')}</span><span>{fmtMoney(grossSubtotal, purSym)}</span></div>
                  <div className="flex justify-between text-success-700 dark:text-success-400"><span>{t('Discount')}</span><span>− {fmtMoney(lineDiscount, purSym)}</span></div>
                </>
              )}
              ...
              {(purchase.shipping || 0) > 0.005 && (
                <div className="flex justify-between text-slate-600 dark:text-slate-300"><span>{t('Freight')}</span><span>{fmtMoney(purchase.shipping, purSym)}</span></div>
              )}
              {purchase.taxAmount > 0 && (
                <div className="flex justify-between text-slate-600 dark:text-slate-300"><span>{settings.tax.name}</span><span>{fmtMoney(purchase.taxAmount, purSym)}</span></div>
              )}
              ...
              {(purchase.amountPaid > 0 || debited > 0) && (
                <>
                  {purchase.amountPaid > 0 && <div className="flex justify-between text-success-700 dark:text-success-400"><span>{t('Amount Paid')}</span><span>({fmtMoney(purchase.amountPaid, purSym)})</span></div>}
                  ...
                  <div className="flex justify-between font-bold text-warning-700 dark:text-warning-400 border-t border-slate-200 dark:border-surface-700 pt-2">
          ...
          {purchase.notes && (
            <div className="mt-6 pt-6 border-t border-slate-100 dark:border-surface-750">
              <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase mb-1">{t('Notes')}</p>
              <p className="text-sm text-slate-600 dark:text-slate-300 whitespace-pre-line">{purchase.notes}</p>
            </div>
          )}
          ...
          {je && (
            <div className="mt-6 pt-6 border-t border-slate-100 dark:border-surface-750 no-print">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-xs text-slate-500 dark:text-slate-400 uppercase border-b border-slate-100 dark:border-surface-700">
                ...
                <tbody>
                  {(je.lines || []).map((l, i) => (
                    <tr key={i} className="border-b border-slate-50 dark:border-surface-750">
                      <td className="py-2 text-slate-700 dark:text-slate-200">{accName(l.accountId)}</td>
                      <td className="py-2 text-end font-mono text-slate-700 dark:text-slate-200">{l.debit > 0 ? fmtMoney(l.debit, sym) : '—'}</td>
                      <td className="py-2 text-end font-mono text-slate-700 dark:text-slate-200">{l.credit > 0 ? fmtMoney(l.credit, sym) : '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {purchase.payments?.length > 0 && (
            <div className="mt-6 pt-6 border-t border-slate-100 dark:border-surface-750 no-print">
              <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase mb-3">{t('Payments')}</p>
              {purchase.payments.map((p) => (
                <div key={p.id} className="flex justify-between text-sm py-1.5 border-b border-slate-50 dark:border-surface-750 last:border-0">
                  <span className="text-slate-600 dark:text-slate-300">{fmtDate(p.date)} · {p.number}{p.notes ? ` · ${p.notes}` : ''}</span>
                  <span className="font-medium text-slate-800 dark:text-slate-100 tabular-nums">{fmtMoney(p.amount, purSym)}</span>
                </div>
              ))}
            </div>
          )}
