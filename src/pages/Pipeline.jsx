...
const STAGES = [
  ...
  { id: 'contacted', label: 'Contacted',  color: 'border-t-brand-400' },
  { id: 'proposal',  label: 'Proposal',   color: 'border-t-warning-400' },
  { id: 'won',       label: 'Won',        color: 'border-t-success-500' },
  { id: 'lost',      label: 'Lost',       color: 'border-t-danger-400' },
]
...
export default function Pipeline() {
  ...
  const convert = async (lead) => {
    if (await ask(`Convert "${lead.name}" to a customer?`)) {
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-5 gap-4">
        {STAGES.map((stage) => {
          ...
          return (
            <div key={stage.id} className={`bg-slate-50 dark:bg-slate-800/50 rounded-xl border-t-4 ${stage.color} p-3`}>
              <div className="flex items-center justify-between mb-3 px-1">
                <p className="text-sm font-semibold text-slate-700 dark:text-slate-200">{t(stage.label)}</p>
                <span className="text-xs text-slate-500 dark:text-slate-400">{items.length}</span>
              </div>
              <p className="text-xs text-slate-500 dark:text-slate-400 px-1 mb-2">{fmtMoney(total, sym)}</p>
              <div className="space-y-2 min-h-[40px]">
                {items.map((l) => (
                  <div key={l.id} className="bg-white dark:bg-slate-800 rounded-lg border border-slate-100 dark:border-slate-700 p-3 shadow-sm">
                    <div className="flex items-start justify-between">
                      <div className="min-w-0">
                        <p className="font-medium text-slate-800 dark:text-slate-100 text-sm truncate">{l.name}</p>
                        {l.company && <p className="text-xs text-slate-500 dark:text-slate-400 truncate">{l.company}</p>}
                      </div>
                      <span className="text-xs font-semibold text-success-700 dark:text-success-400 whitespace-nowrap">{fmtMoney(l.value || 0, sym)}</span>
                    </div>
                    ...
                    <div className="flex items-center justify-between mt-2 pt-2 border-t border-slate-50 dark:border-slate-700/50">
                      <div className="flex gap-1">
                        <button onClick={() => move(l, -1)} disabled={l.stage === 'new'} className="text-slate-500 dark:text-slate-400 hover:text-brand-600 dark:hover:text-brand-400 disabled:opacity-30"><ChevronLeft size={14} /></button>
                        <button onClick={() => move(l, 1)} disabled={l.stage === 'lost'} className="text-slate-500 dark:text-slate-400 hover:text-brand-600 dark:hover:text-brand-400 disabled:opacity-30"><ChevronRight size={14} /></button>
                      </div>
                      <div className="flex gap-1 items-center">
                        <AttachmentButton entityType="lead" entityId={l.id} />
                        {l.stage !== 'won' && <button onClick={() => convert(l)} title="Convert to customer" className="text-slate-500 dark:text-slate-400 hover:text-success-600"><UserPlus size={13} /></button>}
                        <button onClick={() => openEdit(l)} className="text-slate-500 dark:text-slate-400 hover:text-brand-600 dark:hover:text-brand-400"><Pencil size={12} /></button>
                        <button onClick={async () => { if (await ask('Delete this lead?')) deleteLead(l.id) }} className="text-slate-500 dark:text-slate-400 hover:text-danger-500"><Trash2 size={12} /></button>
                      </div>
                    </div>
                  </div>
                ))}
                {items.length === 0 && <p className="text-xs text-slate-500 dark:text-slate-400 text-center py-3">—</p>}
              </div>
            </div>
          )
        })}
