    ...
    <div>
      ...
      <Card className="p-5 mb-6">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="font-semibold text-sm text-slate-700 dark:text-slate-200">{t('Quick Converter')}</h3>
        </div>
        <div className="flex flex-wrap items-end gap-3">
          ...
          <div className="bg-brand-50 dark:bg-brand-900/30 rounded-lg px-4 py-2">
            <p className="text-[11px] text-brand-600 dark:text-brand-300 uppercase">Result</p>
            <p className="text-lg font-bold text-brand-700 dark:text-brand-200">{convert().toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} {to}</p>
          </div>
        </div>
      </Card>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {COMMON.filter((c) => c.code !== base && !currencies.some((x) => x.code === c.code)).slice(0, 10).map((c) => (
          <button key={c.code} onClick={() => quickAdd(c)} className="text-xs px-2.5 py-1 rounded-full border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 hover:border-brand-400 hover:text-brand-600">
            + {c.code}
          </button>
        ))}
      </div>
      ...
      <Card>
        {currencies.length === 0 ? (
          <div className="py-12 text-center">
            <Coins size={30} className="mx-auto mb-3 text-slate-500 dark:text-slate-400" />
            <p className="text-slate-500 dark:text-slate-400 text-sm">{t('No foreign currencies yet. Add one above, or click a suggestion.')}</p>
          </div>
        ) : (
            ...
            <tbody>
              {currencies.map((c) => (
                <tr key={c.id} className="border-b border-slate-50 dark:border-slate-700/50">
                  <td className="px-5 py-2.5 font-mono font-semibold text-slate-800 dark:text-slate-100">{c.code}</td>
                  <td className="px-4 py-2.5 text-slate-600 dark:text-slate-300">{c.name}</td>
                  <td className="px-4 py-2.5 text-right font-medium text-slate-800 dark:text-slate-100">{c.rate}</td>
                  <td className="px-4 py-2.5 text-slate-500 dark:text-slate-400 text-xs">{c.updatedAt ? fmtDate(c.updatedAt.slice(0, 10)) : '—'}</td>
                  <td className="px-5 py-2.5 text-right">
                    <div className="flex justify-end gap-1">
                      <Btn size="sm" variant="ghost" onClick={() => openEdit(c)}><Pencil size={13} /></Btn>
                      <Btn size="sm" variant="ghost" onClick={async () => { if (await ask(`Remove ${c.code}?`)) deleteCurrency(c.id) }}><Trash2 size={13} className="text-danger-600 dark:text-danger-400" /></Btn>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editing ? 'Edit currency' : 'Add currency'}>
        <div className="space-y-4">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">Example: if 1 {base} = 3.75 SAR, set the SAR rate to 3.75.</p>
