...
import { ask, notify } from '../components/Dialogs'
...
export default function Settings() {
   ...
   */
  const confirmAnchor = async (data) => {
    const res = checkBackupAnchor(data)
    if (!res || res.ok) return true
    return await ask(t("This backup's ledger does not match the seal it was exported with — it has been changed since it was created. Restore it anyway?"))
  }
    ...
    reader.onload = async (ev) => {
      try {
        ...
        if (parsed.encrypted) {
          ...
          if (!(await confirmAnchor(parsed.data))) return
          if (!await ask(t('Restoring will REPLACE all current data with the backup. Continue?'))) return
          ...
          await flushNow()
          await notify(t('Backup restored successfully! The page will reload.'))
  ...
  const handleDecryptImport = async () => {
    ...
    try {
      ...
      if (!(await confirmAnchor(data))) { setImportBusy(false); return }
      if (!await ask(t('Restoring will REPLACE all current data with the backup. Continue?'))) { setImportBusy(false); return }
      importData(data)
  ...
  const handleRestoreSnap = async (id) => {
    if (!await ask(t('Restoring will REPLACE all current data with this snapshot. Continue?'))) return
    await restoreSnapshot(id)
  }

  const handleDeleteSnap = async (id) => {
    if (!await ask(t('Delete this snapshot?'))) return
  ...
  const handleReset = async () => {
    if (await ask(t('This will permanently erase ALL data (invoices, customers, transactions, etc.). Are you absolutely sure?'))) {
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Company Info */}
        <Card className="p-6">
          <h2 className="text-base font-semibold text-slate-800 dark:text-slate-100 mb-4">{t('Company Information')}</h2>
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea
                rows={3}
                className="w-full border border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-brand-500 focus:ring-4 focus:ring-brand-500/15 transition-all duration-150 resize-none"
        ...
        <div className="space-y-5">
          <Card className="p-6">
            <h2 className="text-base font-semibold text-slate-800 dark:text-slate-100 mb-4">{t('Currency & Fiscal Year')}</h2>
            <div className="space-y-4">
          ...
          <Card className="p-6">
            <h2 className="text-base font-semibold text-slate-800 dark:text-slate-100 mb-4">{t('Inventory Costing')}</h2>
          ...
          <Card className="p-6">
            <h2 className="text-base font-semibold text-slate-800 dark:text-slate-100 mb-4">{t('Tax Settings')}</h2>
            <div className="space-y-4">
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">{t('Fills in the tax name, rate and currency below — every field stays editable after.')}</p>
                {findTaxRegion(tax.country)?.note && (
                  <p className="text-xs text-warning-700 dark:text-warning-400 mt-1.5 bg-warning-50 dark:bg-warning-900/20 rounded-lg px-2.5 py-1.5">{t(findTaxRegion(tax.country).note)}</p>
                )}
              </div>
              <div className="flex items-center gap-3">
                <input
                  ...
                  className="w-4 h-4 rounded text-brand-600 dark:text-brand-400 focus:ring-brand-500"
                />
                <label htmlFor="taxEnabled" className="text-sm font-medium text-slate-700 dark:text-slate-300">{t('Enable Tax (VAT / GST / Sales Tax)')}</label>
              </div>
              {tax.enabled && (
                <>
        ...
        {/* Invoice Settings */}
        <Card className="p-6">
          <h2 className="text-base font-semibold text-slate-800 dark:text-slate-100 mb-4">{t('Invoice Settings')}</h2>
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{t('Default Invoice Notes')}</label>
              <textarea
                rows={3}
                className="w-full border border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-brand-500 focus:ring-4 focus:ring-brand-500/15 transition-all duration-150 resize-none"
                value={invoice.notes}
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{t('Bank Details / Payment Instructions')}</label>
              <textarea
                rows={3}
                className="w-full border border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-100 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-brand-500 focus:ring-4 focus:ring-brand-500/15 transition-all duration-150 resize-none"
        ...
        <Card className="p-6">
          <div className="flex items-center gap-2.5 mb-4">
            ...
            <h2 className="text-base font-semibold text-slate-800 dark:text-slate-100">{t('AI Assistant')}</h2>
          </div>
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{t('Claude API Key')}</label>
              <div className="relative">
                <input
                  ...
                  className="w-full border border-slate-300 dark:border-surface-600 rounded-lg px-3 py-2 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent font-mono"
                />
                <button onClick={() => setShowKey(v => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 dark:text-slate-400 hover:text-slate-600 dark:hover:text-slate-300">
        ...
        {/* Invoice Branding */}
        <Card className="p-6">
          <h2 className="text-base font-semibold text-slate-800 dark:text-slate-100 mb-4">{t('Invoice Branding')}</h2>
          <div className="space-y-4">
            ...
            <BrandingPanel />
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">{t('Accent Color')}</label>
              <div className="flex items-center gap-3">
                <input type="color" value={company.accentColor || '#2563eb'} onChange={(e) => setCompanyField('accentColor', e.target.value)} className="w-12 h-9 rounded border border-slate-200 dark:border-slate-600 cursor-pointer" />
                <span className="text-sm text-slate-500 dark:text-slate-400 font-mono">{company.accentColor || '#2563eb'}</span>
              </div>
            </div>
          </div>
        </Card>
        ...
        <Card className="p-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <input type="checkbox" id="zatcaEnabled" checked={zatca.enabled} onChange={(e) => setZatcaField('enabled', e.target.checked)} className="w-4 h-4 rounded text-success-700 dark:text-success-400 focus:ring-success-500" />
              <label htmlFor="zatcaEnabled" className="text-sm font-medium text-slate-700 dark:text-slate-300">{t('Enable ZATCA e-invoicing')}</label>
            </div>
            {zatca.enabled && (
              <>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div className="flex items-center gap-3">
                  <input type="checkbox" id="zatcaQr" checked={zatca.showQr} onChange={(e) => setZatcaField('showQr', e.target.checked)} className="w-4 h-4 rounded text-success-700 dark:text-success-400 focus:ring-success-500" />
                  <label htmlFor="zatcaQr" className="text-sm font-medium text-slate-700 dark:text-slate-300">{t('Show ZATCA QR code on invoices')}</label>
                </div>
                <div className="bg-success-50 dark:bg-success-900/20 rounded-lg p-3 text-xs text-success-700 dark:text-success-300 space-y-1">
        ...
        <Card className="p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <h2 className="text-base font-semibold text-slate-800 dark:text-slate-100">{t('Setup guide')}</h2>
        ...
        <Card className="p-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <input type="checkbox" id="etaEnabled" checked={eta.enabled} onChange={(e) => setEtaField('enabled', e.target.checked)} className="w-4 h-4 rounded text-warning-700 dark:text-warning-400 focus:ring-warning-500" />
              <label htmlFor="etaEnabled" className="text-sm font-medium text-slate-700 dark:text-slate-300">{t('Enable ETA e-invoicing')}</label>
            </div>
            {eta.enabled && (
              <>
                ...
                <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide pt-1">{t('Issuer address')}</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide pt-1">{t('VAT sub-type codes')}</p>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div className="bg-warning-50 dark:bg-warning-900/20 rounded-lg p-3 text-xs text-warning-700 dark:text-warning-300 space-y-1">
        ...
        {/* Withholding Tax */}
        <Card className="p-6">
          <h2 className="text-base font-semibold text-slate-800 dark:text-slate-100 mb-1">{t('Withholding Tax (WHT)')}</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">Deduct withholding tax on supplier payments (e.g. KSA WHT on payments to non-residents). The withheld amount is posted to a “Withholding Tax Payable” account to remit later.</p>
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <input type="checkbox" id="whtEnabled" checked={wht.enabled} onChange={(e) => setWhtField('enabled', e.target.checked)} className="w-4 h-4 rounded text-brand-600 dark:text-brand-400 focus:ring-brand-500" />
              <label htmlFor="whtEnabled" className="text-sm font-medium text-slate-700 dark:text-slate-300">{t('Enable withholding tax on payments')}</label>
            </div>
            {wht.enabled && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <Card className="p-6">
          <div className="flex items-center gap-2.5 mb-4">
            ...
            <h2 className="text-base font-semibold text-slate-800 dark:text-slate-100">{t('Period Close / Lock')}</h2>
          </div>
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
          ...
          {settings.accounting?.lockDate ? (
            <div className="rounded-lg bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 px-4 py-3 mb-4 text-sm">
              ...
              {settings.accounting.lockedBy && <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">{t('Locked by')} {settings.accounting.lockedBy}</p>}
            </div>
          ) : (
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-4">{t('No period is currently locked — all dates are open for posting.')}</p>
          )}
          {isManager ? (
        ...
        <Card className="p-6">
          <div className="flex items-center gap-2.5 mb-4">
            <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-warning-500 to-warning-600 flex items-center justify-center">
              <ShieldCheck size={14} className="text-white" />
            </div>
            <h2 className="text-base font-semibold text-slate-800 dark:text-slate-100">{t('Approvals')}</h2>
          </div>
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
          ...
          {isManager ? (
            <>
              <label className="flex items-start gap-3 cursor-pointer mb-4">
                <input type="checkbox" className="mt-0.5 h-4 w-4 rounded border-slate-300 dark:border-slate-600"
                  checked={!!approvals.enabled}
                  onChange={(e) => updateApprovals({ enabled: e.target.checked })} />
                <span className="text-sm">
                  <span className="font-medium text-slate-800 dark:text-slate-100">{t('Require approval above a threshold')}</span>
                  <span className="block text-slate-500 dark:text-slate-400 mt-0.5">{t('Applies to new documents only — anything already posted is unaffected.')}</span>
                </span>
              </label>
              ...
              <div className={`space-y-3 ${approvals.enabled ? '' : 'opacity-50 pointer-events-none'}`}>
                {Object.entries(APPROVAL_KINDS).map(([kind, cfg]) => (
                  <div key={kind} className="flex flex-wrap items-center gap-3 justify-between">
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-slate-800 dark:text-slate-100">{t(cfg.label)}</p>
                      <p className="text-xs text-slate-500 dark:text-slate-400">{t(cfg.hint)}</p>
                    </div>
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <span className="text-xs text-slate-500 dark:text-slate-400">{settings.company.currencySymbol}</span>
                ...
                <label className="flex items-start gap-3 cursor-pointer pt-2 border-t border-slate-100 dark:border-surface-750">
                  <input type="checkbox" className="mt-0.5 h-4 w-4 rounded border-slate-300 dark:border-slate-600"
                    checked={approvals.requireDifferentUser !== false}
                    onChange={(e) => updateApprovals({ requireDifferentUser: e.target.checked })} />
                  <span className="text-sm">
                    <span className="font-medium text-slate-800 dark:text-slate-100">{t('The submitter cannot approve their own request')}</span>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-3">{t('Approval settings apply immediately — no need to press Save.')}</p>
            </>
          ) : (
            <span className="text-xs text-slate-500 dark:text-slate-400">{t('Owners / Admins only')}</span>
          )}
        ...
        <Card className="p-6">
          <div className="flex items-center gap-2.5 mb-4">
            ...
            <h2 className="text-base font-semibold text-slate-800 dark:text-slate-100">{t('Automation')}</h2>
          </div>
          <label className="flex items-start gap-3 cursor-pointer">
            ...
            />
            <span className="text-sm">
              <span className="font-medium text-slate-800 dark:text-slate-100">{t('Auto-post recurring entries on start-up')}</span>
              <span className="block text-slate-500 dark:text-slate-400 mt-0.5">
        ...
        <Card className="p-6">
          <div className="flex items-center gap-2.5 mb-4">
            <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-success-500 to-teal-600 flex items-center justify-center">
              <Database size={14} className="text-white" />
            </div>
            <h2 className="text-base font-semibold text-slate-800 dark:text-slate-100">{t('Backup & Restore')}</h2>
          </div>
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
          ...
          <label className="flex items-center gap-2.5 mb-3 cursor-pointer select-none">
            <input type="checkbox" checked={encryptExport} onChange={(e) => setEncryptExport(e.target.checked)}
              className="h-4 w-4 rounded border-slate-300 dark:border-slate-600 text-brand-600 dark:text-brand-400 focus:ring-brand-500" />
            <Shield size={14} className="text-brand-600 dark:text-brand-400" />
            <span className="text-sm font-medium text-slate-700 dark:text-slate-200">{t('Encrypt backup with passphrase')}</span>
          </label>
          {encryptExport && (
            <div className="mb-4 flex items-end gap-3">
              ...
              <p className="text-xs text-warning-700 dark:text-warning-400 mb-1 max-w-xs">{t('If you forget the passphrase, the backup cannot be recovered.')}</p>
            </div>
          )}
          ...
          <div className="mt-5 pt-4 border-t border-slate-100 dark:border-slate-700">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">{t('Local Snapshots')}</span>
                <span className="text-xs text-slate-500 dark:text-slate-400">({t('auto-saved daily, max 8')})</span>
              </div>
            ...
            {snapshotsOpen && (
              <div className="space-y-2">
                {snapshots.length === 0 && <p className="text-xs text-slate-500 dark:text-slate-400">{t('No snapshots yet — one will be created automatically tomorrow.')}</p>}
                {snapshots.map((s) => (
                  <div key={s.id} className="flex items-center justify-between rounded-lg bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 px-3 py-2 text-sm">
                    <div>
                      <span className={`inline-block px-1.5 py-0.5 rounded text-[10px] font-semibold uppercase me-2 ${s.label === 'Auto' ? 'bg-brand-100 text-brand-700 dark:bg-brand-900/40 dark:text-brand-300' : 'bg-success-100 text-success-700 dark:bg-success-900/40 dark:text-success-300'}`}>{s.label}</span>
                      <span className="text-slate-600 dark:text-slate-300">{new Date(s.at).toLocaleString()}</span>
                      <span className="text-slate-500 dark:text-slate-400 ms-2 text-xs">~{Math.round(s.bytes / 1024)} KB</span>
                    </div>
                    <div className="flex gap-1">
                      ...
                      <button onClick={() => handleDeleteSnap(s.id)} className="px-2 py-1 rounded text-xs font-medium text-danger-600 dark:text-danger-400 hover:bg-danger-50 dark:hover:bg-danger-900/30 transition-colors" title={t('Delete')}>
                        <Trash2 size={13} />
                      </button>
                    </div>
                  </div>
          ...
          <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-700">
            <div className="flex justify-between text-xs text-slate-500 dark:text-slate-400 mb-1">
              <span>{t('Browser storage used')} <span className="text-slate-500 dark:text-slate-400">({t('IndexedDB')})</span></span>
              <span>~{storageKB.toLocaleString()} KB {quotaKB ? `of ~${quotaKB.toLocaleString()} KB` : ''}</span>
            </div>
            <div className="h-2 rounded-full bg-slate-200 dark:bg-slate-700 overflow-hidden">
              <div className={`h-full ${storagePct > 85 ? 'bg-danger-500' : storagePct > 60 ? 'bg-warning-500' : 'bg-success-500'}`} style={{ width: `${storagePct}%` }} />
            </div>
            {storagePct > 85 && <p className="text-xs text-danger-600 dark:text-danger-400 mt-1">{t('Storage is nearly full — download a backup and consider archiving old data.')}</p>}
          </div>
        </Card>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={() => { setImportModal(null); setImportPass(''); setImportErr('') }}>
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-elevated w-full max-w-sm mx-4 p-6" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-base font-semibold text-slate-800 dark:text-slate-100">{t('Encrypted Backup')}</h3>
              </div>
              <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">{t('This backup is encrypted. Enter the passphrase to restore it.')}</p>
              ...
              {importErr && <p className="text-sm text-danger-600 dark:text-danger-400 mt-2">{importErr}</p>}
        ...
        <Card className="p-6 border-danger-200/70 dark:border-danger-500/25">
          <div className="flex items-start gap-3 mb-4">
            <AlertTriangle size={18} className="text-danger-600 dark:text-danger-400 mt-0.5 flex-shrink-0" />
            <div>
              <h2 className="text-base font-semibold text-danger-700 dark:text-danger-400">{t('Danger Zone')}</h2>
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">{t('These actions are irreversible. Please proceed with caution.')}</p>
            </div>
          </div>
          <div className="space-y-3">
            <div className="flex items-center justify-between py-3 border-b border-slate-100 dark:border-surface-750">
              <div>
