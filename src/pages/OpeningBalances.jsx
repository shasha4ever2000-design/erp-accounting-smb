...
export default function OpeningBalances() {
  ...
  const handleReverse = async () => {
    if (!await ask(t('Undo the opening balances? The entry, the carried-in documents and the opening stock are all removed.'))) return
    try {
  ...
  return (
    <div>
      ...
      {posted ? (
        <Card className="p-5 mb-6 bg-success-50/50 dark:bg-success-500/[0.07] ring-1 ring-inset ring-success-500/20">
          <div className="flex items-start gap-3">
            <CheckCircle2 size={20} className="text-success-700 dark:text-success-400 flex-shrink-0 mt-0.5" />
      ...
      <Card className="p-5 mb-6">
        <div className="flex flex-wrap items-end gap-4">
          ...
          <div className="ms-auto text-end">
            <p className="text-xs text-slate-500 dark:text-slate-400 uppercase tracking-wider">{t('Difference to allocate')}</p>
            <p className={`text-2xl font-bold tabular-nums ${summary.balanced ? 'text-success-700 dark:text-success-400' : 'text-warning-700 dark:text-warning-400'}`}>
              {fmtMoney(Math.abs(entry.plug), sym)}
            </p>
          </div>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4 pt-4 border-t border-slate-100 dark:border-surface-750 text-sm">
          <div><p className="text-xs text-slate-500 dark:text-slate-400">{t('Total debits')}</p><p className="font-semibold tabular-nums text-slate-800 dark:text-slate-100">{fmtMoney(entry.debits, sym)}</p></div>
          <div><p className="text-xs text-slate-500 dark:text-slate-400">{t('Total credits')}</p><p className="font-semibold tabular-nums text-slate-800 dark:text-slate-100">{fmtMoney(entry.credits, sym)}</p></div>
          <div><p className="text-xs text-slate-500 dark:text-slate-400">{t('Receivables')}</p><p className="font-semibold tabular-nums text-slate-800 dark:text-slate-100">{fmtMoney(entry.ar, sym)}</p></div>
          <div><p className="text-xs text-slate-500 dark:text-slate-400">{t('Payables')}</p><p className="font-semibold tabular-nums text-slate-800 dark:text-slate-100">{fmtMoney(entry.ap, sym)}</p></div>
        </div>
      ...
      <Card className="overflow-hidden">
        <div className="p-4 border-b border-slate-100 dark:border-surface-750 flex flex-wrap gap-1.5">
          {TABS.map((x) => (
            <button key={x.id} onClick={() => setTab(x.id)}
              className={`px-3.5 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                tab === x.id ? 'bg-brand-600 text-white shadow-card' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-surface-750'
              }`}>
              {t(x.label)}
            </button>
          ))}
              ...
              <Table headers={['Account', 'Type', { label: `Opening balance (${sym})`, right: true }]}>
                {glAccounts.map((a) => {
                  ...
                  return (
                    <Tr key={a.id}>
                      <Td>
                        <span className="font-mono text-[11px] text-slate-500 dark:text-slate-400 me-2">{a.code}</span>
                        {a.name}
                      </Td>
                      ...
                      <Td right>
                        {derived ? (
                          <span className="inline-flex items-center gap-2">
                            <span className="text-xs text-slate-500 dark:text-slate-400">{t('from the {x} tab').replace('{x}', t(derived))}</span>
                            <span className="tabular-nums font-medium text-slate-700 dark:text-slate-200">{fmtMoney(derivedValue, sym)}</span>
                          </span>
                        ) : (
