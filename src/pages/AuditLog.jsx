...
const SEVERITY = {
  critical: { chip: 'bg-danger-50 text-danger-700 dark:bg-danger-900/25 dark:text-danger-300', icon: AlertTriangle, label: 'Critical' },
  notable:  { chip: 'bg-warning-50 text-warning-700 dark:bg-warning-900/25 dark:text-warning-300', icon: Pencil, label: 'Change' },
    ...
    <div>
      ...
      {(auditLog || []).length === 0 ? (
        ...
        <>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mb-3">{rows.length} {t('of')} {(auditLog || []).length} {t('events')}</p>
          <Card className="overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody>
                {rows.map((e) => (
                  <tr key={e.id} className="border-b border-slate-50 dark:border-slate-700/50">
                    <td className="px-5 py-2.5 text-slate-500 dark:text-slate-400 whitespace-nowrap">{fmtTs(e.ts)}</td>
                    <td className="px-4 py-2.5">
                      <span className="inline-flex items-center gap-1.5 text-slate-700 dark:text-slate-200">
                        <ShieldCheck size={13} className="text-success-700 dark:text-success-400" /> {e.user}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-2.5 font-medium text-slate-800 dark:text-slate-100 capitalize">
                      {e.action}
                      {e.entityRef && <span className="block text-xs font-normal text-slate-500 dark:text-slate-400 font-mono">{e.entityRef}</span>}
                    </td>
                        ...
                        <div className="space-y-0.5">
                          {e.changes.slice(0, 4).map((c, i) => (
                            <div key={i} className="text-xs">
                              <span className="text-slate-500 dark:text-slate-400">{c.field}</span>{' '}
                              <span className="text-danger-600/80 dark:text-danger-400/80 line-through">{c.from}</span>{' → '}
                              <span className="text-success-700 dark:text-success-400">{c.to}</span>
                            </div>
                          ))}
                          {e.changes.length > 4 && <div className="text-xs text-slate-500 dark:text-slate-400">+{e.changes.length - 4} {t('more')}</div>}
                        </div>
                      ) : e.detail}
                    </td>
                  </tr>
                ))}
                {rows.length === 0 && <tr><td colSpan={5} className="px-5 py-8 text-center text-slate-500 dark:text-slate-400">{t('No matching activity')}</td></tr>}
              </tbody>
            </table>
          </Card>
        </>
