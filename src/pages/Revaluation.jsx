...
export default function Revaluation() {
  ...
  const post = async () => {
    ...
    if (!await ask(t('Post FX revaluation dated {d}?').replace('{d}', date))) return
    try {
    ...
    <div>
      ...
      <Card className="p-4 mb-6 flex flex-wrap items-end gap-4">
        ...
        <div className="ms-auto text-end">
          <p className="text-xs text-slate-500 dark:text-slate-400">{t('Net unrealized')}</p>
          <p className={`text-lg font-bold ${totalDelta >= 0 ? 'text-success-700 dark:text-success-400' : 'text-danger-600 dark:text-danger-400'}`}>
      ...
      {fxAccounts.length === 0 ? (
        ...
        <Card className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {computed.map((r) => (
                <tr key={r.acc.id} className="border-b border-slate-50 dark:border-slate-700/50">
                  <td className="py-2 px-3 text-slate-700 dark:text-slate-200"><span className="font-mono text-xs text-slate-500 dark:text-slate-400 me-2">{r.acc.code}</span>{r.acc.name}</td>
                  ...
                  <td className="py-2 px-3 text-end">
                    <input type="number" step="0.01" value={rows[r.acc.id]?.fcBalance ?? ''} onChange={(e) => setCell(r.acc.id, 'fcBalance', e.target.value)}
                      className="w-28 text-end border border-slate-200 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 rounded px-2 py-1 text-sm focus:outline-none focus:border-brand-500 focus:ring-4 focus:ring-brand-500/15 transition-all duration-150" />
                  </td>
                  <td className="py-2 px-3 text-end">
                    <input type="number" step="0.0001" value={rows[r.acc.id]?.rate ?? ''} onChange={(e) => setCell(r.acc.id, 'rate', e.target.value)}
                      className="w-24 text-end border border-slate-200 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 rounded px-2 py-1 text-sm focus:outline-none focus:border-brand-500 focus:ring-4 focus:ring-brand-500/15 transition-all duration-150" />
                  </td>
                  <td className="py-2 px-3 text-end text-slate-600 dark:text-slate-300">{fmtMoney(r.currentBase, sym)}</td>
                  <td className="py-2 px-3 text-end font-medium text-slate-800 dark:text-slate-100">{fmtMoney(r.revaluedBase, sym)}</td>
                  <td className={`py-2 px-3 text-end font-semibold ${r.gl > 0 ? 'text-success-700 dark:text-success-400' : r.gl < 0 ? 'text-danger-600 dark:text-danger-400' : 'text-slate-500 dark:text-slate-400'}`}>
              ...
              <tr className="border-t-2 border-slate-300 dark:border-slate-500 bg-slate-50/60 dark:bg-slate-700/40 font-bold">
                ...
                <td className={`py-2 px-3 text-end ${totalDelta >= 0 ? 'text-success-700 dark:text-success-400' : 'text-danger-700 dark:text-danger-400'}`}>{totalDelta >= 0 ? '' : '('}{fmtMoney(Math.abs(totalDelta), sym)}{totalDelta >= 0 ? '' : ')'}</td>
              </tr>
            </tbody>
          </table>
        </Card>
      ...
      {fxRevaluations.length > 0 && (
        <Card className="mt-6">
          <div className="p-4 border-b border-slate-100 dark:border-slate-700 font-semibold text-slate-700 dark:text-slate-200">{t('Revaluation History')}</div>
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 text-slate-500 dark:text-slate-400 text-xs uppercase">
            ...
            <tbody>
              {[...fxRevaluations].reverse().map((r) => (
                <tr key={r.id} className="border-b border-slate-50 dark:border-slate-700/50">
                  <td className="py-2 px-3 text-slate-600 dark:text-slate-300">{fmtDate(r.date)}</td>
                  <td className="py-2 px-3 text-slate-600 dark:text-slate-300">{r.note || '—'}</td>
                  <td className="py-2 px-3 text-slate-500 dark:text-slate-400">{r.reversalDate ? fmtDate(r.reversalDate) : '—'}</td>
                  <td className="py-2 px-3 text-end text-slate-600 dark:text-slate-300">{r.entries.length}</td>
                  <td className={`py-2 px-3 text-end font-semibold ${r.gainLoss >= 0 ? 'text-success-700 dark:text-success-400' : 'text-danger-600 dark:text-danger-400'}`}>{r.gainLoss >= 0 ? '' : '('}{fmtMoney(Math.abs(r.gainLoss), sym)}{r.gainLoss >= 0 ? '' : ')'}</td>
                </tr>
              ))}
            </tbody>
          </table>
