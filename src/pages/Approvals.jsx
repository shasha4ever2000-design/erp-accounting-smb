...
export default function Approvals() {
  ...
  const decide = async (req, action) => {
    ...
    if (action === 'approve') {
      if (!await ask(t('Approve this request? It will post to the ledger immediately.'))) return
      try {
  ...
  const TabBtn = ({ id, label, count }) => (
    <button onClick={() => setTab(id)}
      className={`px-3.5 py-1.5 rounded-lg text-sm font-medium transition-colors ${
        tab === id ? 'bg-brand-600 text-white shadow-card' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-surface-750'
      }`}>
      {label}{count > 0 && <span className={`ms-1.5 text-xs ${tab === id ? 'opacity-80' : 'text-slate-500 dark:text-slate-400'}`}>{count}</span>}
    </button>
  )
    ...
    <div>
      ...
      {!approvals.enabled && (
        <Card className="p-4 mb-6 flex items-start gap-3 bg-warning-50/60 dark:bg-warning-500/[0.08] ring-1 ring-inset ring-warning-500/20">
          <Clock size={18} className="text-warning-700 dark:text-warning-400 flex-shrink-0 mt-0.5" />
      ...
      <Card className="overflow-hidden">
        ...
        {rows.length === 0 ? (
          <EmptyState icon={<ShieldCheck size={28} className="text-slate-500 dark:text-slate-400" />} title="Nothing here"
              ...
              return (
                <Tr key={r.id}>
                  ...
                  <Td>
                    ...
                    {r.resultRef && <span className="ms-2 text-xs text-slate-500 dark:text-slate-400">→ {r.resultRef}</span>}
                  </Td>
                    ...
                    {r.status === 'pending' && (
                      <div className="flex items-center gap-1.5 justify-end">
                        {allowed.ok ? (
                          ...
                          <span className="text-xs text-slate-500 dark:text-slate-400">{t(allowed.reason)}</span>
                        )}
                        {isMine && (
...
function RequestDetail({ req, sym, t }) {
  const p = req.payload || {}
  const Row = ({ k, v }) => (
    <div className="flex justify-between gap-4 py-1.5 text-sm border-b border-slate-100 dark:border-surface-750 last:border-0">
      <span className="text-slate-500 dark:text-slate-400">{k}</span>
      <span className="text-slate-800 dark:text-slate-100 text-end">{v}</span>
    </div>
  )
  return (
    <div className="space-y-4">
      ...
      {Array.isArray(p.items) && p.items.length > 0 && (
        <div>
          <h4 className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-1.5">{t('Lines')}</h4>
          {p.items.map((l, i) => (
            <div key={i} className="flex justify-between gap-4 py-1 text-sm">
              <span className="text-slate-700 dark:text-slate-200">{l.description || l.name} <span className="text-slate-500 dark:text-slate-400">× {l.quantity}</span></span>
              <span className="tabular-nums text-slate-800 dark:text-slate-100">{fmtMoney(l.subtotal, sym)}</span>
            </div>
          ))}
        </div>
      )}
      ...
      {Array.isArray(p.lines) && p.lines.length > 0 && (
        <div>
          <h4 className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-1.5">{t('Journal lines')}</h4>
          {p.lines.map((l, i) => (
            <div key={i} className="flex justify-between gap-4 py-1 text-sm">
              <span className="text-slate-700 dark:text-slate-200">{l.description || l.accountId}</span>
