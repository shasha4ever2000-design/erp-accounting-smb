...
export default function FinancialHealth() {
  ...
  const scoreColor = score == null ? 'text-slate-500 dark:text-slate-400' : score >= 75 ? 'text-success-700 dark:text-success-400' : score >= 50 ? 'text-brand-600 dark:text-brand-400' : score >= 30 ? 'text-warning-700 dark:text-warning-400' : 'text-danger-600 dark:text-danger-400'
        ...
        <div className="flex flex-col sm:flex-row items-center gap-6">
          <div className="relative flex-shrink-0" style={{ width: 128, height: 128 }}>
            <svg viewBox="0 0 120 120" className="w-32 h-32 -rotate-90">
              <circle cx="60" cy="60" r="52" fill="none" strokeWidth="12" className="stroke-slate-100 dark:stroke-surface-700" />
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              ...
              {score != null && <span className="text-[11px] text-slate-500 dark:text-slate-400">/ 100</span>}
            </div>
          </div>
          <div className="text-center sm:text-start">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1 max-w-md">{t('A weighted read across liquidity, profitability, efficiency and leverage. Not a substitute for professional advice.')}</p>
          ...
          return (
            <Card key={g.group} className="p-5">
              <div className="flex items-center gap-2.5 mb-1">
                ...
                <h3 className="font-semibold text-slate-800 dark:text-slate-100">{t(g.group)}</h3>
              </div>
              <p className="text-xs text-slate-500 dark:text-slate-400 mb-4 ps-10">{t(g.blurb)}</p>
              <div className="divide-y divide-slate-50 dark:divide-surface-700/50">
                {g.metrics.map((m) => {
                    ...
                    <div key={m.key} className="py-3 flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-medium text-slate-800 dark:text-slate-100 text-sm">{t(m.label)}</span>
                        </div>
                        <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1 ps-4">{m.formula}</p>
                        <p className="text-[11px] text-slate-500 dark:text-slate-400 ps-4">{t(m.hint)}</p>
                      </div>
                      <div className="text-end flex-shrink-0">
                        <div className="text-lg font-bold text-slate-900 dark:text-slate-100 tabular">{display}</div>
