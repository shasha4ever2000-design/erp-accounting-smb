...
export default function DataImport() {
  ...
  const handleApply = async () => {
    ...
    if (!await ask(t('Import {n} record(s) into {e}?').replace('{n}', n).replace('{e}', t(schema.label)))) return
    ...
    <div>
      ...
      <Card className="p-5 mb-6">
        ...
        {entity === 'inventoryItems' && (
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-3 flex items-start gap-2">
      ...
      {!file && !result && (
        <Card>
          <EmptyState
            icon={<Upload size={28} className="text-slate-500 dark:text-slate-400" />}
      ...
      {file && plan && (
        <>
          {/* Column mapping */}
          <Card className="p-5 mb-6">
            <h3 className="font-semibold text-slate-800 dark:text-slate-100 mb-1">{t('Column mapping')}</h3>
            ...
            <div className="flex flex-wrap items-center gap-4">
              ...
              <div className="flex items-center gap-3">
                <label className="flex items-center gap-2 text-sm cursor-pointer">
                  ...
                  <span className="text-slate-700 dark:text-slate-200">{t('Update records that already exist')}</span>
                </label>
          ...
          {(plan.creates.length > 0 || plan.updates.length > 0) && (
            <Card className="overflow-hidden">
              <div className="p-4 border-b border-slate-100 dark:border-surface-750">
                <h3 className="font-semibold text-slate-800 dark:text-slate-100">{t('Preview')}</h3>
              </div>
                    ...
                    <Td className="text-xs">
                      {u.changes.map((c) => (
                        <span key={c.field} className="inline-block me-3">
                          ...
                          <span className="text-slate-500 dark:text-slate-400 line-through">{String(c.from) || '—'}</span>{' '}
                          <span className="text-slate-800 dark:text-slate-100">{String(c.to)}</span>
                        </span>
                      ))}
                    </Td>
                  </Tr>
