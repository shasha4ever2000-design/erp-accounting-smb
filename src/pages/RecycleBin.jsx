...
export default function RecycleBin() {
  ...
  const handlePurge = async (entry) => {
    if (!await ask(t('Delete "{n}" for good? This cannot be undone.').replace('{n}', describeEntry(entry)))) return
    purgeFromBin(entry.id)
  }

  const handleEmpty = async () => {
    if (!recycleBin.length) return
    if (!await ask(t('Permanently delete all {n} item(s)? This cannot be undone.').replace('{n}', recycleBin.length))) return
    emptyRecycleBin()
  }
  ...
  return (
    <div>
      ...
      {recycleBin.length === 0 ? (
        <Card>
          <EmptyState
            icon={<Archive size={28} className="text-slate-500 dark:text-slate-400" />}
          ...
          <div className="p-4 border-b border-slate-100 dark:border-surface-750 flex flex-wrap gap-1.5">
            <button onClick={() => setFilter('all')}
              className={`px-3.5 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                filter === 'all' ? 'bg-brand-600 text-white shadow-card' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-surface-750'}`}>
            ...
            {Object.entries(counts).sort((a, b) => b[1] - a[1]).map(([entity, n]) => (
              <button key={entity} onClick={() => setFilter(entity)}
                className={`px-3.5 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                  filter === entity ? 'bg-brand-600 text-white shadow-card' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-surface-750'}`}>
            ...
            {rows.map((e) => {
              ...
              return (
                <Tr key={e.id}>
                  ...
                  <Td className="text-slate-800 dark:text-slate-100">{describeEntry(e)}</Td>
                  <Td className="text-slate-500 dark:text-slate-400">{e.deletedByName}</Td>
                  <Td>
