...
export default function AuthScreen() {
  ...
  return (
    <div className="min-h-screen flex relative bg-slate-50 dark:bg-slate-950">
      <button
        onClick={toggleLang}
        className="absolute top-4 end-4 z-10 inline-flex items-center gap-1.5 bg-white/80 dark:bg-slate-800/80 backdrop-blur border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-1.5 text-sm font-medium text-slate-600 dark:text-slate-300 shadow-card hover:bg-white dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-slate-100 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-slate-950"
      >
        <Globe size={15} /> {lang === 'ar' ? 'English' : 'العربية'}
      </button>

      {/* Brand panel */}
      <div className="hidden lg:flex w-1/2 relative overflow-hidden bg-gradient-to-br from-brand-600 via-accent-600 to-violet-700 dark:from-brand-800 dark:via-accent-900 dark:to-slate-950 text-white p-12 xl:p-16 flex-col justify-between">
        ...
        <div aria-hidden="true" className="pointer-events-none absolute inset-0">
          ...
          <div className="absolute bottom-0 end-0 w-[34rem] h-[34rem] translate-x-1/3 translate-y-1/3 rtl:-translate-x-1/3 rounded-full bg-accent-400/20 dark:bg-accent-500/10 blur-3xl" />
        ...
        <div className="relative max-w-lg">
          ...
          <p className="text-brand-100/90 text-lg leading-relaxed">{t('Double-entry accounting, invoicing, inventory, payroll, projects and POS — with VAT/GST, sales tax, and Saudi ZATCA e-invoicing built in. Built for small & medium businesses anywhere.')}</p>
          <div className="mt-9 space-y-3.5 text-sm">
            {[
              ...
              <p key={f} className="flex items-center gap-3 text-brand-50/95">
        ...
        <p className="relative text-brand-200/70 text-xs">© {new Date().getFullYear()} ERP Accounting</p>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 sm:p-10 relative">
        ...
        <div className="relative w-full max-w-md animate-scale-in">
          <div className="lg:hidden flex items-center gap-2.5 mb-8 justify-center">
            ...
            <span className="text-lg font-bold tracking-tight text-slate-800 dark:text-slate-100">{t('ERP Accounting')}</span>
          </div>

          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-elevated ring-1 ring-black/5 dark:ring-white/10 border border-slate-100 dark:border-slate-800 p-7 sm:p-9">
            <h2 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-100 mb-1.5">{mode === 'signup' ? t('Create your account') : t('Welcome back')}</h2>
            <p className="text-slate-500 dark:text-slate-400 text-sm mb-7">{mode === 'signup' ? t('Sign up to start managing your companies.') : t('Sign in to continue.')}</p>

            <form onSubmit={submit} className="space-y-3.5">
              {mode === 'signup' && (
                <Field icon={<User size={16} />}>
                  <input className="w-full bg-transparent outline-none text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500" aria-label={t('Full name')} placeholder={t('Full name')} value={form.name} onChange={(e) => set('name', e.target.value)} autoFocus />
                </Field>
              )}
              <Field icon={<Mail size={16} />}>
                <input className="w-full bg-transparent outline-none text-sm text-slate-900 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500" aria-label={t('Email')} type="email" placeholder={t('Email')} value={form.email} onChange={(e) => set('email', e.target.value)} />
              </Field>
              ...
              {error && (
                <p role="alert" className="text-sm text-danger-700 dark:text-danger-300 bg-danger-50 dark:bg-danger-900/25 border border-danger-100 dark:border-danger-900/40 rounded-xl px-3.5 py-2.5 animate-fade-in">{error}</p>
              )}

              <button
                type="submit"
                disabled={busy}
                className="w-full bg-gradient-to-b from-brand-600 to-brand-700 hover:from-brand-700 hover:to-brand-800 active:from-brand-800 active:to-brand-800 disabled:opacity-60 disabled:cursor-not-allowed text-white rounded-xl px-4 py-3 text-sm font-semibold flex items-center justify-center gap-2 shadow-btn-primary transition-all duration-150 ease-spring active:scale-[.99] focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-slate-900"
              >
          ...
          <div className="mt-5 flex items-start gap-2.5 text-xs leading-relaxed text-slate-500 dark:text-slate-400 bg-white/70 dark:bg-slate-900/60 backdrop-blur border border-slate-100 dark:border-slate-800 rounded-xl px-4 py-3 shadow-card">
...
function PasswordMeter({ password, t }) {
  ...
  const tone = ['bg-danger-500', 'bg-warning-500', 'bg-warning-500', 'bg-lime-500', 'bg-success-500'][score]
  const text = ['text-danger-600 dark:text-danger-400', 'text-warning-700 dark:text-warning-400',
    'text-warning-700 dark:text-warning-400', 'text-lime-600 dark:text-lime-400',
    'text-success-700 dark:text-success-400'][score]
  ...
  return (
    <div className="animate-fade-in" aria-live="polite">
      <div className="flex gap-1.5" role="img" aria-label={`${t('Password strength')}: ${t(scoreLabel(score))}`}>
        {[0, 1, 2, 3].map((i) => (
          <div key={i} className={`h-1 flex-1 rounded-full transition-colors ${i < score ? tone : 'bg-slate-200 dark:bg-slate-700'}`} />
        ))}
      </div>
      <p className={`text-xs mt-1.5 ${text}`}>
        <span className="font-medium">{t(scoreLabel(score))}</span>
        {hint && <span className="text-slate-500 dark:text-slate-400"> · {t(hint)}</span>}
      </p>
    </div>
  )
}
...
function Field({ icon, children }) {
  return (
    <div className="flex items-center gap-2.5 border border-slate-300 dark:border-slate-600 rounded-xl px-3.5 py-2.5 bg-white dark:bg-slate-800/80 transition-shadow duration-150 focus-within:border-brand-500 dark:focus-within:border-brand-500 focus-within:ring-[3px] focus-within:ring-brand-500/20">
