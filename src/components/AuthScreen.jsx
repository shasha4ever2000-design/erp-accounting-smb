...
import { TrendingUp, Mail, Lock, User, Loader2, ShieldCheck } from 'lucide-react'
  ...
  return (
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex w-1/2 bg-gradient-to-br from-blue-600 via-indigo-600 to-violet-700 text-white p-12 flex-col justify-between">
        ...
        <div>
          <h1 className="text-4xl font-black leading-tight mb-4">Run your whole business in one place.</h1>
      ...
      <div className="flex-1 flex items-center justify-center p-6 bg-slate-50">
        <div className="w-full max-w-sm">
          ...
          <p className="text-gray-500 text-sm mb-6">{mode === 'signup' ? 'Sign up to start managing your companies.' : 'Sign in to continue.'}</p>

          <form onSubmit={submit} className="space-y-3">
            {mode === 'signup' && (
              <Field icon={<User size={16} />}>
                <input className="auth-in" placeholder="Full name" value={form.name} onChange={(e) => set('name', e.target.value)} autoFocus />
              </Field>
            )}
            <Field icon={<Mail size={16} />}>
              <input className="auth-in" type="email" placeholder="Email" value={form.email} onChange={(e) => set('email', e.target.value)} />
            </Field>
            <Field icon={<Lock size={16} />}>
              <input className="auth-in" type="password" placeholder="Password" value={form.password} onChange={(e) => set('password', e.target.value)} onKeyDown={(e) => e.key === 'Enter' && submit(e)} />
            </Field>
