    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-4xl max-h-[90vh] bg-white dark:bg-surface-850 rounded-2xl shadow-modal ring-1 ring-black/5 dark:ring-white/10 flex flex-col animate-scale-in overflow-hidden">
          ...
          <div className="flex items-start justify-between ps-6 pe-4 pt-5 pb-4">
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">{t('Statement of Account')}</span>
              </div>
              <h2 className="mt-1 text-xl font-bold tracking-tight text-slate-900 dark:text-slate-50 truncate">
                {account.code ? <span className="font-mono text-slate-500 dark:text-slate-400 text-base me-2">{account.code}</span> : null}
                {account.name}
              </h2>
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">{companyName}</p>
            </div>
            <button onClick={onClose} aria-label={t('Close')} className="w-8 h-8 flex-shrink-0 flex items-center justify-center rounded-lg text-slate-500 dark:text-slate-400 hover:text-slate-700 hover:bg-slate-100 dark:hover:text-slate-100 dark:hover:bg-white/[0.07] transition-colors">
              <X size={18} />
            </button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {data.rows.length === 0 ? (
            <div className="py-16 text-center text-slate-500 dark:text-slate-400 text-sm">{t('No transactions for this account in the selected period')}</div>
          ) : (
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                <tr className="border-b border-slate-50 dark:border-surface-800/60 text-slate-500 dark:text-slate-400">
                  ...
                  <tr key={r.id} className="border-b border-slate-50 dark:border-surface-800/60 hover:bg-slate-50/70 dark:hover:bg-surface-800/40 transition-colors">
                    ...
                    <td className="px-2 py-2 align-top">
                      ...
                      <div className="flex flex-wrap items-center gap-1.5 mt-1">
                        {r.ref && <span className="font-mono text-[11px] text-slate-500 dark:text-slate-400">{r.ref}</span>}
        ...
        <div className="border-t border-slate-100 dark:border-surface-750 bg-slate-50/70 dark:bg-surface-900/40 px-6 py-3.5 flex items-center justify-between">
          <div className="flex gap-6 text-sm">
            <span className="text-slate-500 dark:text-slate-400">{t('Total Debit')} <span className="font-mono tabular-nums text-slate-700 dark:text-slate-200 ms-1">{fmtMoney(data.totalDr, sym)}</span></span>
            <span className="text-slate-500 dark:text-slate-400">{t('Total Credit')} <span className="font-mono tabular-nums text-slate-700 dark:text-slate-200 ms-1">{fmtMoney(data.totalCr, sym)}</span></span>
          </div>
