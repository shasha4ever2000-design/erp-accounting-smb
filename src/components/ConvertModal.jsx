    ...
    <Modal open={open} onClose={onClose} title={title} width="max-w-2xl">
      {doc && (
        <div className="space-y-4">
                ...
                {items.map((l) => {
                  ...
                  return (
                    <tr key={l.id} className="border-b border-slate-50 dark:border-surface-700/50">
                      ...
                      <td className="py-2 text-end px-2 text-slate-500 dark:text-slate-400 tabular-nums">{l.quantity}</td>
                      <td className="py-2 text-end px-2 text-slate-500 dark:text-slate-400 tabular-nums">{done || '—'}</td>
                      <td className="py-2 text-end px-2 font-medium text-slate-700 dark:text-slate-200 tabular-nums">{remaining}</td>
                      <td className="py-2 ps-2">
                        {remaining > 0 ? (
                          <input type="number" min="0" max={remaining} step="any" value={qty[l.id] ?? ''}
                            ...
                            className="w-24 text-end border border-slate-300 dark:border-surface-600 bg-white dark:bg-surface-800 text-slate-900 dark:text-slate-100 rounded-lg px-2 py-1.5 focus:outline-none focus:border-brand-500 focus:ring-4 focus:ring-brand-500/15" />
                        ) : (
                          <span className="block text-end text-xs text-success-700 dark:text-success-400 pe-1">{t('Fully done')}</span>
                        )}
                      </td>
                    </tr>
                  )
          ...
          <div className="flex items-center justify-between pt-3 border-t border-slate-100 dark:border-surface-700">
            <span className="text-sm text-slate-500 dark:text-slate-400">{t('This conversion')}</span>
            <span className="text-lg font-bold text-slate-900 dark:text-slate-100 tabular-nums">{fmtMoney(preview.total, sym)}</span>
          </div>
