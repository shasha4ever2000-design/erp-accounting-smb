...
const TONE = {
  over:   { bar: 'bg-danger-500',    chip: 'bg-danger-50 text-danger-700 dark:bg-danger-900/25 dark:text-danger-300',       label: 'Over budget' },
  warn:   { bar: 'bg-warning-500',  chip: 'bg-warning-50 text-warning-700 dark:bg-warning-900/25 dark:text-warning-300', label: 'Spending fast' },
    ...
    <div className="mb-6 rounded-2xl bg-white dark:bg-surface-850 ring-1 ring-black/5 dark:ring-white/10 shadow-card overflow-hidden">
      <div className="px-5 py-3 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <AlertTriangle size={15} className="text-warning-700 dark:text-warning-400" />
          <h2 className="text-sm font-semibold text-slate-800 dark:text-slate-100">
            {t('Budget alerts')} <span className="text-slate-500 dark:text-slate-400 font-normal">({alerts.length})</span>
          </h2>
        </div>
          ...
          return (
            <div key={a.id} className="px-5 py-3">
              <div className="flex items-center justify-between gap-3 mb-1.5">
                <span className="flex items-center gap-2 min-w-0">
                  {a.severity === 'behind'
                    ...
                    : <AlertTriangle size={13} className={a.severity === 'over' ? 'text-danger-600 dark:text-danger-400 flex-shrink-0' : 'text-warning-700 dark:text-warning-400 flex-shrink-0'} />}
              ...
              <div className="relative h-2">
                <div className="absolute top-0 w-px h-1.5 bg-slate-400 dark:bg-slate-500" style={{ insetInlineStart: `${Math.min(100, a.elapsed)}%` }} />
              </div>
