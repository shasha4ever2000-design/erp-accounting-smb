      ...
      {showHelp && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4" onClick={() => setShowHelp(false)}>
          ...
          <div className="relative bg-white dark:bg-surface-850 rounded-2xl shadow-modal ring-1 ring-black/5 dark:ring-white/10 w-full max-w-md p-6 animate-scale-in" onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setShowHelp(false)} className="absolute top-4 right-4 text-slate-500 dark:text-slate-400 hover:text-slate-600 dark:hover:text-slate-300"><X size={18} /></button>
            <div className="flex items-center gap-2.5 mb-3">
              ...
              <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">{t('Install the desktop app')}</h2>
            </div>
            <p className="text-sm text-slate-600 dark:text-slate-300 mb-4">Install ERP Accounting so it opens in its own window with a desktop icon — and works offline.</p>
            <div className="space-y-3 text-sm text-slate-600 dark:text-slate-300">
              <div>
                <p className="font-semibold text-slate-800 dark:text-slate-100">Chrome / Edge (Windows, Mac, Linux)</p>
              ...
              <div>
                <p className="font-semibold text-slate-800 dark:text-slate-100">Safari (Mac)</p>
                <p>{t('Open the')}<strong>Share</strong> menu → <strong>{t('Add to Dock')}</strong>.</p>
              </div>
              <div>
                <p className="font-semibold text-slate-800 dark:text-slate-100">iPhone / iPad</p>
